...
#' @export
#'
comment_code <- function(question, code = clipr::read_clip(allow_non_interactive = TRUE)) {
  code <- paste(gsub('"', "'", code), collapse = "\n")
  prompt <- paste0('Add inline comments to the following R code: "', code, '"')
  question[[length(question)]]$content <- paste0('Add inline comments to the following R code. Do not include any other text. Only reply with the altered code wrapped between ```: "', question[[length(question)]]$content, '"')
